...
class XMLFile(object):
    ...
    def __init__(self, XMLfilename = "default.xml"):
        ...
        root.text = ("XML file which contains the full set of PIRENEA parameters")
    ...
    def fill_general(self, root):
        ...
        ET.SubElement(parent, "date_time").text = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    
    def add_pulse(self, parent, channel, start_time, duration, function):
        
        child = ET.SubElement(parent, self.d["pulse"])
        ET.SubElement(child, self.d["channel"], number=self.ch[channel]).text = channel
        ET.SubElement(child, self.d["start_time"], units="ms").text = "{:.1f}".format(float(start_time))
        ET.SubElement(child, self.d["duration"], units="ms").text = "{:.1f}".format(float(duration))
    ...
    def fill_sequence(self, root):
        ...
        ET.SubElement(parent, "sequence_filename").text = ("NationalInstruments.bin")
        
        """ Trapping  Pulses """
        child = self.add_pulse(parent, self.d["Gin"], "400", "500", self.d["trapping"])
        child = self.add_pulse(parent, self.d["GI_in"], "600", "700", self.d["trapping"])
...
#         baby = ET.SubElement(child, "buffer").text = ("Excitation Buffer")
        ...
        ET.SubElement(baby, "buffer_filename").text = ("swift_1.2_80_250_300.buf")
...
#             if len(pulse):
    ...
    def remove_pulse(self, tree, channel, start_time, duration, function):
...
#     import xml.etree.cElementTree as ET
    ...
    toto.remove_pulse(tree, 1, 4000, 100, "adiabatic")
else:
    print("\nImporting... ", __name__)
